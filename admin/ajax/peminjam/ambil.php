...
$table = <<<EOT
 (
    SELECT 
      ...
      a.nama_peminjam, 
      a.username,
      b.nama_jabatan as jabatan,
      c.nama_kelas as kelas
    FROM peminjam a LEFT JOIN jabatan b ON a.id_jabatan = b.id_jabatan LEFT JOIN kelas c ON a.id_kelas = c.id_kelas
 ) temp
EOT;
 
// Table's primary key
$primaryKey = 'id_peminjam';
...
// indexes
$columns = array(
    array( 'db' => 'id_peminjam', 'dt' => 0 ),
    array( 'db' => 'nama_peminjam',  'dt' => 1 ),
    array( 'db' => 'jabatan',   'dt' => 2 ),
    array( 'db' => 'kelas', 'dt' => 3 ),
    array( 'db' => 'username', 'dt' => 4 ),

    array( 'db' => 'id_peminjam', 'dt' => 5, 'formatter' => function( $d, $row ) {
